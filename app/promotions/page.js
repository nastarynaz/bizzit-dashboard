...
export default function PromotionsPage() {
  ...
  const calendarEvents = [
    { id: 1, title: "Design Conference", date: 2, color: "bg-purple-200 text-purple-800" },
    { id: 2, title: "Weekend Festival", date: 16, color: "bg-pink-200 text-pink-800" },
    { id: 3, title: "Glastonbury Festival", date: 20, color: "bg-orange-200 text-orange-800", span: 2 },
    { id: 4, title: "Glastonbury Festival", date: 24, color: "bg-blue-200 text-blue-800" },
  ]

  const toggleStatus = (productId) => {
    setProducts(
      products.map((product) =>
        product.id === productId ? { ...product, status: product.status === "aktif" ? "non aktif" : "aktif" } : product,
      ),
    )
  }

  const activeProducts = products.filter((product) => product.status === "aktif")
  const recommendedProducts = products.filter((product) => product.status === "non aktif")
  ...
  const renderCalendarDay = (day) => {
    ...
    const event = calendarEvents.find((e) => e.date === dayNumber && isCurrentMonth)

    return (
      <div
        key={day.toISOString()}
        className={`min-h-20 p-1 border border-gray-100 ${!isCurrentMonth ? "text-gray-300" : ""}`}
      >
        <div className="text-sm font-medium mb-1">{dayNumber}</div>
        {event && <div className={`text-xs p-1 rounded ${event.color} truncate`}>{event.title}</div>}
      </div>
    )
  }
        ...
        <tbody>
          {productList.map((product, index) => (
            <tr key={product.id} className="border-b hover:bg-muted/50">
              ...
              <td className="p-3 text-sm font-medium text-green-600">{formatCurrency(product.potentialRevenue)}</td>
              <td className="p-3">
                {showStatusToggle ? (
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-1">
                {["Day", "Week", "Month"].map((view) => (
                  <Button
                    ...
                    className={calendarView === view ? "bg-blue-500 text-white" : ""}
                  >
                    {view}
                  </Button>
                ))}
            ...
            <Card>
              <CardContent className="p-0">
                ...
                <div className="grid grid-cols-7">{generateCalendarDays().map(renderCalendarDay)}</div>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              {activeProducts.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">Belum ada promosi yang aktif</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
